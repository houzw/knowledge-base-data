...
class SwatArticleSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for node in node_list:
			...
			item['primary_application_category'] = node.xpath('./tr[12]/td[2]/text()').extract()
			item['secondary_application_category'] = node.xpath('./tr[13]/td[2]/text()').extract()
			item['watershed_description'] = node.xpath('./tr[14]/td[2]/text()').extract()
			item['calibration_summary'] = node.xpath('./tr[15]/td[2]/text()').extract()
			item['validation_summary'] = node.xpath('./tr[16]/td[2]/text()').extract()
			item['general_comments'] = node.xpath('./tr[17]/td[2]/text()').extract()
			abs_com = node.xpath('./tr[18]/td[1]/strong/text()').extract_first()
			if abs_com != 'Abstract:':
				item['language'] = node.xpath('./tr[18]/td[2]/text()').extract()
				item['keywords'] = node.xpath('./tr[19]/td[2]/text()').extract()
			else:
				item['abstract'] = node.xpath('./tr[18]/td[2]/text()').extract()
				item['language'] = node.xpath('./tr[19]/td[2]/text()').extract()
				item['keywords'] = node.xpath('./tr[20]/td[2]/text()').extract()
			yield item
